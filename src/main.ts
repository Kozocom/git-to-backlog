    ...
    switch (pr.action) {
      ...
      case "closed":
        if (pr.pull_request.merged) {
          payload = {
            // statusId: 3,
            comment: `Pull request [#${pr.number}](${pr.pull_request.html_url}) merged and closed by ${pr.pull_request}: ${title}`,
          };
        } else {
          payload = {
            comment: `Pull request [#${pr.number}](${pr.pull_request.html_url}) closed by ${pr.pull_request}: ${title}`,
          };
        }
        break;
      default:
